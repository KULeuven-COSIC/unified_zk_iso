...
def optimised_strategy_with_first_eval(n,mul_c=1,first_eval_c=1,precomp_dbl=1):
    ...
    S_left, _, _, _ = optimised_strategies_with_first_eval_new(n,mul_c,first_eval_c,precomp_dbl)

    return S_left[n]

@cached_function
def optimised_strategies_with_first_eval_new(n,mul_c=1,first_eval_c=1,precomp_dbl=1):
    r"""
    Adapted from optimised_strategy when the fist isogeny evaluation is more costly.
...
def precompute_strategy_with_first_eval(e,m,M=1,S=0.8,I=100):
    ...
    precomp_dbl = 3*(45*M)+16*S

    return optimised_strategy_with_first_eval(n, mul_c = mul_c/eval_c, first_eval_c = first_eval_c/eval_c, precomp_dbl = precomp_dbl/eval_c)

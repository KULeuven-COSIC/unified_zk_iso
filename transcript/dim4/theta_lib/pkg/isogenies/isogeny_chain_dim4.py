...
class IsogenyChainDim4:
    ...
    def get_strategy(self,splitting):
        ...
        #else:
        return precompute_strategy_with_first_eval(self.e,self.m,M=1,S=0.8,I=10)
    ...
    def isogeny_chain(self, B_K, first_isogenies):
        ...
        for k in range(n):
            ...
            Th = phi._codomain
            _coords = Th.null_point().coords()
            _zk_str = ';'.join(f'a{j}={_coords[j]}' for j in range(len(_coords)))
            _zk_str += ';\n'
            zk_radical += _zk_str

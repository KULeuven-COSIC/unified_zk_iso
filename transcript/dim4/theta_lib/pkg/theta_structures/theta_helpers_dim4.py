...
def proj_batch_inversion(L):
	...
	Output:
	- [lambda/x for x in L] for unknown lambda
	"""
	# Given L=[a0,...,an]
	# Computes multiples=[a0, a0.a1, ..., a0...an]
	multiples=[L[0]]
	...
	# Computes projective inverses=[1,an,a(n-1)*an,...,a1...an]
	inverses=[1]
	for i in range(1,len(L)):
		inverses.append(inverses[-1]*L[-i])

	# Finally computes projective inverses 
	# lambda[1/a0,...,1/an]=[a1...an,a0a2...an,...,a0...a(n-1)]
	result=[inverses[-1]]
	for i in range(2,len(L)+1):
		result.append(inverses[-i]*multiples[i-2])
	return result

def proj_batch_inversion_with_coeff(L):
	r"""Does n inversions in 3(n-1)M+1I.

...
class ThetaStructureDim4:
	def __init__(self,null_point,null_point_dual=None,inv_null_point_dual=None):
		r"""
		INPUT:
		...
		"""
		if null_point is None and null_point_dual is None and inv_null_point_dual is None:
			raise ValueError("This theta structure contains no data.")
		elif null_point is not None and len(null_point) != 16:
		...
		elif null_point_dual is not None:
			self._base_ring = cm.common_parent(*(c.parent() for c in null_point_dual))
		elif inv_null_point_dual is not None:
			L = []
			for c in inv_null_point_dual:
				if c is not None:
					L.append(c.parent())
			self._base_ring = cm.common_parent(*L)
		self._point = ThetaPointDim4
		if null_point is not None:
			self._null_point = self._point(self, null_point)
		else:
			self._null_point = None
	...
	def precompute_null_point(self):
		if self._null_point is None:
			if self._null_point_dual is not None:
				self._null_point = self._point(self, hadamard(self._null_point_dual))
			elif self._inv_null_point is not None:
				self._null_point = self._point(self, proj_batch_inversion(self._inv_null_point))
			elif self._inv_null_point_dual is not None:
				self._null_point_dual = proj_batch_inversion(self._inv_null_point_dual)
				self._null_point = self._point(self, hadamard(self._null_point_dual))
			else:
				raise ValueError("This theta structure contains no data (this exception should never happen).")

	def null_point(self):
		"""
		"""
		self.precompute_null_point()
	...
	def _arithmetic_base_change(self,max_iter=50):
		...
		if all([HSHO[k]!=0 for k in range(16)]) and all([HO[k]!=0 for k in range(16)]):
			...
			self._inv_null_point=proj_batch_inversion(HO)
			self._inv_null_point_dual_sq=proj_batch_inversion(HSHO)
		else:
			F=self._base_ring
			if F.degree() == 2:
			...
			while count<max_iter:
				...
				if all([NU_chi_0_sq[k]!=0 for k in range(16)]) and all([NO[k]!=0 for k in range(16)]):
					...
					self._inv_null_point=proj_batch_inversion(NO)
					self._inv_null_point_dual_sq=proj_batch_inversion(NU_chi_0_sq)
					break

...
class ThetaIsogeny(Morphism):
    ...
    def _compute_codomain(self, T1, T2):
        ...
        """
        # ZK: Input
        zk_trans = f'x1={T1.coords()[0]};y1={T1.coords()[1]};'
        zk_trans += f'z1={T1.coords()[2]};w1={T1.coords()[3]};'
        zk_trans += f'x2={T2.coords()[0]};y2={T2.coords()[1]};'
        zk_trans += f'z2={T2.coords()[2]};w2={T2.coords()[3]};'
        
        # ZK-radical:
        rad_a0, rad_a1, rad_a2, rad_a3 = self._domain.coords()
        zk_radical = f'a0={rad_a0};a1={rad_a1};a2={rad_a2};a3={rad_a3};'
        rad_x0 = rad_a0**2 + rad_a1**2 + rad_a2**2 + rad_a3**2
        #rad_x0, rad_x1, rad_x2, rad_x3 = ThetaPoint.to_squared_theta(*self._domain.coords())

        #zk_trans_radical = f'{self._domain};'
        ...
        # ZK: Hadamard and squares
        zk_trans += f'{xA=};{xB=};'
        zk_trans += f'{zA=};{tB=};{zC=};{tD=};'
        ...
        # TODO: Only doing this for now, write eq for both
        if True:
            # Batch invert denominators
            ...
            #projective anyway, needed for rad
            rad_x0_sqrt = rad_x0.sqrt() #this choice is arbitrary
            A, B, C, D = rad_x0_sqrt*A, rad_x0_sqrt*B, rad_x0_sqrt*C, rad_x0_sqrt*D
            B_inv = xB_inv * xA
            C_inv = zC_inv * zA
            ...
            rad_y2 = C
            rad_y3 = D
            #zk_radical += f'y0={rad_y0};y1={rad_y1};y2={rad_y2};y3={rad_y3};'
        ...
        # ZK: inverse and multiply
        zk_trans += f'{xA_inv=};{zA_inv=};{tB_inv=};'
        zk_trans += f'{xB_inv=};{zC_inv=};{tD_inv=};'
        zk_trans += f'{A=};{B=};{C=};{D=};{B_inv=};{C_inv=};{D_inv=};'
        ...
        if self._hadamard[1]:
            ...
            # ZK: last hadamard
            zk_trans += f'{a=};{b=};{c=};{d=};'
            with open('zk_trans.txt', 'a') as fh:
                fh.write(zk_trans + '\n')
